...
class DefaultWebhookTransformerUnCompressed extends CLWebhookTransformer {
	...
	override def onNewProduct(settings: WebhookSettings, productId: String, competitionLabsApi: CompetitionLabsApi): Unit = {
		...
		competitionLabsApi.httpPost(settings.url, json, headers, None, sendCompressed = false)
	}
